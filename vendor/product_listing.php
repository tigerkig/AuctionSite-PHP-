               ...
               <div class="panel panel-visible" id="spy3">
                  <div class="panel-heading">
                     <div class="panel-title hidden-xs">
                        <?php echo $lang_187; ?> 
                        <?php
                           //$result=getDetails(doSelect("apa_id,apa_vr_id,name,category,brand,stock,status","ambit_product_add",array("apa_vr_id"=>$_SESSION["vendor_id"]),' ORDER BY apa_id DESC' ));
                           $result=getDetails(doSelect("apa_id,apa_vr_id,name,category,brand,stock,status","ambit_product_add",array("apa_vr_id"=>$_SESSION["vendor_id"], "listing_duration" => '0000:00:00'),' ORDER BY apa_id DESC' ));
                           ?>
                     </div>
                  </div>
                        ...
                        <table class="table table-striped table-hover" id="datatable2" cellspacing="0" width="100%">
                           <thead>
                              <tr>
                                 ...
                                 <th class="va-m">Sold Quantity</th>
                              ...
                              <tr>
                                 ...
                                 <td>
                                    <?php 
                                    	$status = "";
                                       if($v["status"] == 1) {
									   	 if($quantity_sold > 0 && $v["stock"] ==0)
									   	 	$status = '<span style="color:red">Sold</span>';  
									   	 else
									   	 	$status = '<span style="color:green">Active Sale</span>';  
									   }elseif($v["status"] == 0 && $quantity_sold > 0){
									   		$status = '<span style="color:red">Sold</span';  
									   }else{
									   		$status = '<span style="color:red">No Active Sale</span>';  
									   }
                                 ...
                                 <td>
                                    <?php 
                                    	$payment_status = "";
                                       if($quantity_sold > 0) {
                                       	$payment_status = "<span style='color:red'>Paid</span>";
                                       }
                                       else{
                                       	if($quantity_paid > 0)
                                       		$payment_status = "<span style='color:red'>Sold</span>";
                                       	else
                                       		$payment_status = "<span style='color:red'>Unpaid</span>";
                                       }  
                                       
                                       if($return_count > 0){
											$payment_status = '<span style="color:red">Refunded</span>';  
										}
                                       echo $payment_status;
                                       ?>					
                                 </td>
