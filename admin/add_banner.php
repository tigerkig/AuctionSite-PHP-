...
function changeStatus(id){
		$.post("admin_approve_banner.php",{aab_id:id,msg:'approve_banner'},function(r){
			if(r==1){	
		location.reload();	
	}else{
...
function delete_product(id){
		...
		$.post("admin_approve_banner.php",{aab_id:id,msg:'delete_banner'},function(r){
			if(r==1){
		location.reload();	
	}else{
		alert("Something went wrong !");
...
function doUpdate(id){
	...
	$.post("get_product_id.php",{name:link},function(link){
       $.post("admin_approve_banner.php",{aab_id:id,aab_link:link,msg:'update_banner'},function(r){
	   if(r==1){
	   location.reload();	
	 }else{
	 	alert("Something went wrong !");
...
Banner List
<?php
$result=getDetails(doSelect1("aab_id,aab_banner,aab_link,status","ambit_add_banner",array()));
...
<label for="link" class="field prepend-icon">
<select list="browsers" id="product_link<?php echo $v["aab_id"];  ?>" name="product_link" value="" placeholder="Type Product Name" style="height: 40px; width: 180px;">
