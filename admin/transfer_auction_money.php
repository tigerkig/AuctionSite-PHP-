...
$_SESSION["sub_menu"]  = "transfer_auction_money";
...
?>
	...
	function add_money(ass_id, ass_apa_id, money_transfer, ass_vr_id) {
		...
		if(confirm('Do you want to add money to vendor account?')) {
			$.post("ajax_money_to_vendor_auction.php", {
				ass_id: ass_id,
				ass_apa_id: ass_apa_id,
				ass_vr_id: ass_vr_id
			}, function(r) {
				//alert(r);
				if(r == 1) {
					alert("Money added to vendor account");
					window.location = "transfer_auction_money.php";
				}
				if(r == 0) {
					alert("Something went wrong !");
					window.location = "transfer_auction_money.php";
	...
	function change_status(id, product_id, quantity) {
		if(confirm('Do you want to change this order status?')) {
			$.post('ajax_stock_check.php', {
				product_id: product_id,
				quantity: quantity
			}, function(r) {
				if(r == 1) {
					$.post("shipment_approve_auction.php", {
						ass_id: id
					}, function(r) {
						location.reload();
					});
		...
		<div id="topbar-dropmenu-wrapper">
			<div class="topbar-menu row">
				<div class="col-xs-4 col-sm-2">
					<a href="#" class="service-box bg-danger"> <span class="fa fa-music"></span> <span class="service-title">Audio</span> </a>
				</div>
				<div class="col-xs-4 col-sm-2">
					<a href="#" class="service-box bg-success"> <span class="fa fa-picture-o"></span> <span class="service-title">Images</span> </a>
				</div>
				<div class="col-xs-4 col-sm-2">
					<a href="#" class="service-box bg-primary"> <span class="fa fa-video-camera"></span> <span class="service-title">Videos</span> </a>
				</div>
				<div class="col-xs-4 col-sm-2">
					<a href="#" class="service-box bg-alert"> <span class="fa fa-envelope"></span> <span class="service-title">Messages</span> </a>
				</div>
				<div class="col-xs-4 col-sm-2">
					<a href="#" class="service-box bg-system"> <span class="fa fa-cog"></span> <span class="service-title">Settings</span> </a>
				</div>
				<div class="col-xs-4 col-sm-2">
					<a href="#" class="service-box bg-dark"> <span class="fa fa-user"></span> <span class="service-title">Users</span> </a>
				</div>
			</div>
		</div>
		<header id="topbar" class="alt">
			<div class="topbar-left">
				<ol class="breadcrumb">
					...
					<li class="breadcrumb-active"> <a href="dashboard1.html">Dashboard</a> </li>
			...
			<div class="topbar-right">
				...
				<div class="ml15 ib va-m" id="sidebar_right_toggle">
					<div class="navbar-btn btn-group btn-group-number mv0">
						<button class="btn btn-sm btn-default btn-bordered prn pln"> <i class="fa fa-bar-chart fs22 text-default"></i> </button>
					</div>
				</div>
			</div>
		</header>
						...
						<div class="col-md-12">
							<div class="panel panel-visible" id="spy6">
								<div class="panel-heading">
									<div class="panel-title hidden-xs"> Transfer Auction Money
										<?php

$result=getDetails(doSelect("ass_id,ass_apa_id,cancel_status,ass_ac_id,ass_vr_id,date,shipment_status,payment_transfer,quantity","ambit_shipment_status",array("auction_flag"=>1)));
...
?>
													<tr>
														...
														<td>
															<?php if($v["delivery_status"]==1){ echo 'Yes';}else{ echo 'No';}  ?>
														</td>
														<td>
															<?php
...
?>
																	<?php if($v["shipment_status"]==1)echo '<img src="icon/tick1.png" width="20">';else echo '<img src="icon/cross.png" width="20" title="Not shipment yet">';  ?>
																		<a href="javascript:void(0);" onclick="change_status('<?php echo $v[" ass_id "];  ?>','<?php echo $v["ass_apa_id "];  ?>','<?php echo $v["quantity "];  ?>');" title="Change Shipment Status"><img src="icon/edit.png" width="13" /></a>
																		<?php
if($v["cancel_status"]==0){
?>
