...
$_SESSION["sub_menu"]  = "change_vendor_password";
...
<li class="breadcrumb-current-item">Change Vendor Password</li>
</ol>
</div>
...
if(type.trim()=="0"){
	...
	$.ajax({
        	url: "ajax_update_vendor_password.php",
			type: "POST",
...
function fetch_email(id,id1){
	var vr_id=$("#"+id).val().trim();
	if(vr_id!=0){
		$.post(
		'ajax_fetch_email.php',
		{vr_id:vr_id,msg:'vendor_email'},
		function(r){
		$("#"+id1).val(r);
		}
		);
...
<select id="vr_id" name="vr_id" onchange="fetch_email('vr_id','email');">
<option value="0">Select Vendor</option>
<?php
$vendor=getDetails(doSelect("vr_id,fname,email","vendor_registration",array()));
foreach($vendor as $k=>$v){
?>
<option value="<?php echo $v["vr_id"]; ?>"><?php echo trim($v["fname"]); ?></option>
<?php
}
?>
</select>
...
<option value="0">Send Mail to vendor???</option>
